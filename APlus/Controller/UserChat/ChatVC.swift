...
public class ChatVC: UIViewController {
    ...
    @IBAction func btnSendTap(_ sender: UIButton) {
        if txtTypeMsg.text! != "" {
            let param : [String : Any] = ["message": txtTypeMsg.text!, "isRead" : false, "type" : "text", "viewBy" : (recentChatUser?.members)!, "readBy" : SocketChatManager.sharedInstance.myUserId, "sentAt" : "", "sentBy" : SocketChatManager.sharedInstance.myUserId, "timeMilliSeconds": "", "replyUser": "", "replyMsg": "", "replyMsgId": ""]
            let param1 : [String : Any] = ["messageObj" : param, "groupId" : groupId, "secretKey" : SocketChatManager.sharedInstance.secretKey, "userId": SocketChatManager.sharedInstance.myUserId, "userName": SocketChatManager.sharedInstance.myUserName]
